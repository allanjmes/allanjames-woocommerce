<!-- <div id="main-content" style="display: none;"> -->
    <!-- HERO -->
    <section class="max-w-7xl xl:mx-auto">
        <div class="flex justify-center">
            <h1 class="text-9xl font-semibold tracking-thin bg-gradient-to-r from-primary via-yellow to-accent bg-clip-text text-transparent leading-normal">
                <span class="font-anuphan">Fujifilm</span> GFX 100S
            </h1>
        </div>
        <div class="my-10"></div>
        <div class="bg-primary">
            <div class="grid grid-cols-1 lg:grid-cols-4">
                <div class="col-span-1 flex flex-col justify-center gap-10 lg:ms-10">
                    <h1 class="text-secondary text-3xl font-light">16 Megapixel HD digital camera</h1>
                    <div class="flex justify-center gap-3">
                        <button class="btn-outline-secondary">Start Shopping</button>
                        <button class="btn-secondary">-></button>
                    </div>
                </div>
                <div class="col-span-2 relative lg:-top-36">
                    <img
                        src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera.png'; ?>"
                        class="relative w-90 h-auto sm:w-120 lg:w-full"
                        alt=""
                    >
                    <div>
                        <p class="text-center text-secondary relative mx-10">Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugiat, maxime dignissimos ad aut doloremque harum at deleniti saepe temporibus id?</p>
                    </div>
                </div>
                <div class="col-span-1 flex flex-col justify-center gap-10 lg:me-10">
                    <h1 class="text-secondary text-3xl font-light">16 Megapixel HD digital camera</h1>
                    <div class="flex justify-center gap-3">
                        <button class="btn-accent">Start Shopping</button>
                        <button class="btn-outline-accent">-></button>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- WHO WE ARE -->
    <section class="max-w-7xl xl:mx-auto block relative">
        <div class="my-20">
            <div class="grid grid-rows-2 lg:grid-rows-none lg:grid-cols-3 gap-10">
                <h1 class="text-9xl col-span-1 text-primary">Who We are</h1>
                <div class="flex col-span-2 items-center">
                    <p class="text-2xl ms-10 me-5 text-gray-500">We believe that every photograph tells a story, and every story deserves to be captured with precision, clarity, and soul. Founded by a team of passionate photographers and engineers</p>
                    <svg xmlns="http://www.w3.org/2000/svg" class="fill-gray-300" width="400px" height="200px" viewBox="0 0 32 32">
                        <path d="M27.719 5.109c-0.135-0.141-0.266-0.286-0.401-0.422-0.141-0.141-0.281-0.271-0.427-0.401-2.953-2.766-6.849-4.297-10.891-4.286-4.245-0.010-8.318 1.677-11.313 4.688-6.104 6.099-6.234 15.938-0.401 22.203 0.13 0.146 0.26 0.286 0.396 0.427 0.141 0.135 0.281 0.266 0.422 0.401 2.958 2.76 6.854 4.292 10.896 4.281 4.276 0 8.292-1.661 11.318-4.682 6.099-6.104 6.229-15.943 0.401-22.208zM5.51 5.51c2.776-2.792 6.552-4.354 10.49-4.344 3.766 0 7.318 1.396 10.068 3.943l-1.333 1.339c-2.385-2.188-5.5-3.401-8.734-3.391-3.438-0.010-6.729 1.354-9.151 3.792-2.438 2.422-3.802 5.719-3.792 9.151 0 3.266 1.198 6.339 3.391 8.734l-1.339 1.333c-5.375-5.813-5.245-14.911 0.401-20.557zM23.906 7.266v0.005l-1.354 1.354c-1.797-1.62-4.135-2.51-6.552-2.505-2.62-0.010-5.135 1.036-6.984 2.896-1.859 1.844-2.901 4.365-2.896 6.984-0.005 2.417 0.885 4.75 2.5 6.552l-1.349 1.354c-1.969-2.161-3.057-4.984-3.047-7.906-0.010-3.125 1.234-6.125 3.448-8.328 2.203-2.214 5.203-3.458 8.328-3.448 2.953 0 5.734 1.078 7.906 3.042zM24.714 16c0 2.328-0.906 4.516-2.552 6.161s-3.833 2.557-6.161 2.557c-2.109 0-4.146-0.771-5.729-2.161v-0.005c-0.146-0.125-0.292-0.25-0.432-0.391-0.135-0.141-0.266-0.286-0.391-0.432-1.396-1.583-2.167-3.62-2.161-5.729 0-2.328 0.906-4.516 2.552-6.161 1.63-1.641 3.849-2.557 6.161-2.552 2.13 0 4.141 0.766 5.729 2.161 0.146 0.125 0.297 0.25 0.432 0.391 0.141 0.141 0.266 0.286 0.391 0.432 1.396 1.583 2.167 3.62 2.161 5.729zM9.448 23.38c1.797 1.615 4.135 2.505 6.552 2.505 2.625 0.005 5.141-1.036 6.984-2.901 1.859-1.849 2.906-4.365 2.896-6.984 0.005-2.417-0.885-4.75-2.5-6.552l1.354-1.354c1.964 2.161 3.052 4.984 3.047 7.906 0.005 3.125-1.234 6.125-3.453 8.328-2.203 2.219-5.203 3.458-8.328 3.453-2.922 0.005-5.745-1.083-7.906-3.047zM26.49 26.49c-2.776 2.792-6.552 4.354-10.49 4.344-3.734 0.010-7.333-1.401-10.068-3.943l1.339-1.333c2.38 2.188 5.5 3.396 8.734 3.391 3.432 0.005 6.729-1.359 9.151-3.797 2.432-2.422 3.802-5.719 3.792-9.151 0.005-3.234-1.203-6.349-3.391-8.729l1.333-1.339c5.375 5.813 5.245 14.911-0.401 20.557z"/>
                    </svg>
                </div>
            </div>
            <div class="grid grid-rows-2 lg:grid-rows-none lg:grid-cols-3 gap-10">
                <div class="col-span-1 flex justify-start relative">
                    <svg class="fill-accent w-24 h-auto transform scale-200 rotate-45 lg:ms-15" viewBox="0 0 100 100" width="100px" height="100px">
                        <path d="M0,50 a1,1 0 0,0 100,0"/>
                    </svg>
                    <?php get_template_part('template/components/star-icon', null, [
                        'class' => 'bg-gray-100 h-20 w-20 relative lg:top-15 lg:-left-10',
                        'svg_class' => 'stroke-black fill-black h-[45px] w-auto'
                    ]); ?>
                </div>
                <div class="bg-yellow col-span-2 lg:h-60 flex justify-center relative">
                    <img
                        src="<?php echo get_template_directory_uri() . '/src/assets/home/video-3d-camera.png'; ?>"
                        class="w-50 h-50 sm:w-60 transform lg:scale-300 relative lg:top-5"
                        alt=""
                    >
                </div>
            </div>
        </div>
    </section>
    <!-- EXCLUSIVE -->
    <section class="max-w-7xl xl:mx-auto my-20">
        <div class="my-20">
            <h1 class="text-9xl col-span-1 text-primary text-center lg:mx-10">Our Exclusive Camera Just For You</h1>
        </div>
        <div class="flex flex-col gap-5">
            <div class="collapse collapse-exclusive collapse-arrow border border-gray-300 rounded-none dark:text-primary">
                <input type="radio" name="my-accordion-1" />
                <div class="collapse-title">
                    <h1 class="font-semibold text-center text-3xl">Phase One XF IQ4 150MP</h1>
                </div>
                <div class="collapse-content text-2xl grid grid-rows-3 lg:grid-rows-none lg:grid-cols-4">
                    <div class="row-span-1 col-span-1 flex justify-start items-end ps-5">
                        <h1>$230.00</h1>
                    </div>
                    <div class="row-span-1 col-span-2 flex justify-center items-center">
                        <img
                            src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera.png'; ?>"
                            class="w-90 h-full"
                            alt=""
                        >
                    </div>
                    <div class="row-span-1 col-span-1 flex justify-end items-end pe-5">
                        <h1>Total Ratings: 5</h1>
                    </div>
                </div>
            </div>
            <div class="collapse collapse-exclusive collapse-arrow active-collapse border border-gray-300 rounded-none dark:text-primary">
                <input type="radio" name="my-accordion-1" checked="checked" />
                <div class="collapse-title">
                    <h1 class="font-semibold text-center text-3xl">Phase One XF IQ4 150MP</h1>
                </div>
                <div class="collapse-content text-2xl grid grid-rows-3 lg:grid-rows-none lg:grid-cols-4">
                    <div class="row-span-1 col-span-1 flex justify-start items-end ps-5">
                        <h1>$230.00</h1>
                    </div>
                    <div class="row-span-1 col-span-2 flex justify-center items-center">
                        <img
                            src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera.png'; ?>"
                            class="w-90 h-full"
                            alt=""
                        >
                    </div>
                    <div class="row-span-1 col-span-1 flex justify-end items-end pe-5">
                        <h1>Total Ratings: 5</h1>
                    </div>
                </div>
            </div>
            <div class="collapse collapse-exclusive collapse-arrow border border-gray-300 rounded-none dark:text-primary">
                <input type="radio" name="my-accordion-1" />
                <div class="collapse-title">
                    <h1 class="font-semibold text-center text-3xl">Phase One XF IQ4 150MP</h1>
                </div>
                <div class="collapse-content text-2xl grid grid-rows-3 lg:grid-rows-none lg:grid-cols-4">
                    <div class="row-span-1 col-span-1 flex justify-start items-end ps-5">
                        <h1>$230.00</h1>
                    </div>
                    <div class="row-span-1 col-span-2 flex justify-center items-center">
                        <img
                            src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera.png'; ?>"
                            class="w-90 h-full"
                            alt=""
                        >
                    </div>
                    <div class="row-span-1 col-span-1 flex justify-end items-end pe-5">
                        <h1>Total Ratings: 5</h1>
                    </div>
                </div>
            </div>
            <div class="collapse collapse-exclusive collapse-arrow border border-gray-300 rounded-none dark:text-primary">
                <input type="radio" name="my-accordion-1" />
                <div class="collapse-title">
                    <h1 class="font-semibold text-center text-3xl">Phase One XF IQ4 150MP</h1>
                </div>
                <div class="collapse-content text-2xl grid grid-rows-3 lg:grid-rows-none lg:grid-cols-4">
                    <div class="row-span-1 col-span-1 flex justify-start items-end ps-5">
                        <h1>$230.00</h1>
                    </div>
                    <div class="row-span-1 col-span-2 flex justify-center items-center">
                        <img
                            src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera.png'; ?>"
                            class="w-90 h-full"
                            alt=""
                        >
                    </div>
                    <div class="row-span-1 col-span-1 flex justify-end items-end pe-5">
                        <h1>Total Ratings: 5</h1>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="bg-primary max-w-7xl xl:mx-auto">
        <div class="p-10">
            <div class="relative">
                <!-- Top-left Image -->
                <img
                    src="<?php echo get_template_directory_uri() . '/src/assets/home/retro-camera.jpg'; ?>"
                    class="absolute top-0 left-0 w-32 lg:w-55 h-auto"
                    alt="Retro Camera Left"
                >
    
                <!-- Bottom-right or mid-right Image -->
                <img
                    src="<?php echo get_template_directory_uri() . '/src/assets/home/female-colorful.jpg'; ?>"
                    class="absolute top-15 right-0 w-32 lg:w-60 h-auto"
                    alt="Retro Camera Right"
                >
    
                <!-- Centered Text -->
                <div class="flex justify-center items-center">
                    <div class="w-90">
                        <h1 class="text-9xl text-secondary text-center px-4">
                            Explore Our Gallery
                        </h1>
                    </div>
                </div>
            </div>
            <div class="grid lg:grid-cols-2 gap-20 pt-10">
                <div class="col-span-1">
                    <div class="flex justify-end">
                        <img
                            src="<?php echo get_template_directory_uri() . '/src/assets/home/man-with-camera.jpg'; ?>"
                            class="lg:w-60 h-auto"
                            alt="Retro Camera Right"
                        >
                    </div>
                </div>
                <div class="col-span-1">
                    <div class="flex justify-between items-center gap-5">
                        <p class="text-secondary text-sm">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quam et eos corrupti ipsum est harum!</p>
                        <button class="btn-accent w-[250px]">
                            Explore Gallery >
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- PRODUCT DETAILS -->
    <section class="max-w-7xl xl:mx-auto">
        <div class="my-20 flex flex-col justify-center items-center">
            <h1 class="text-9xl col-span-1 text-primary lg:mx-10">Our Product</h1>
            <h1 class="text-9xl col-span-1 text-primary lg:mx-10">Details</h1>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-2 lg:grid-rows-[150px_150px_150px] gap-4">
            <!-- Left Large Image -->
            <div class="lg:row-span-3 bg-gray-100 overflow-hidden">
                <div class="flex justify-start relative lg:-top-30 lg:-left-50">
                    <img
                    src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera.png'; ?>"
                    alt="Videoing Lens"
                    class="transform scale-120 object-cover"
                    >
                </div>
            </div>

            <!-- Top Right Image -->
            <div class="lg:row-span-2 bg-gray-100 overflow-hidden">
                <div class="flex justify-end relative lg:-top-30 lg:-right-30">
                    <img
                    src="<?php echo get_template_directory_uri() . '/src/assets/home/video-3d-camera.png'; ?>"
                    alt="Film Lens"
                    class="transform scale-150 object-cover"
                    >
                </div>
            </div>

            <!-- Bottom Right Image -->
            <div class="lg:row-span-1 bg-gray-100 overflow-hidden">
                <div class="flex justify-center align-center relative lg:-top-25">
                    <img
                    src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera-front.png'; ?>"
                    alt="Another Lens"
                    class="transform scale-100 object-cover"
                    >
                </div>
            </div>
        </div>
    </section>
    <!-- CRAFTED FOR CREATIVES -->
    <section class="max-w-7xl xl:mx-auto">
        <div class="my-10 pb-5 border-b border-gray-500">
            <div class="my-20 flex flex-col justify-center items-center">
                <h1 class="text-9xl col-span-1 text-primary lg:mx-10">Crafted for Creatives</h1>
                <h1 class="text-9xl col-span-1 text-primary lg:mx-10">your lens to the world</h1>
            </div>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 py-10">
            <div class="col-span-1">
                <div class="bg-yellow h-[400px] relative">
                    <div class="relative overflow-hidden h-full w-full">
                        <h1 class="text-9xl font-extrabold text-gray-300 z-10 absolute lg:-bottom-10 lg:-right-3">Power</h1>
                    </div>
                    <img
                        src="<?php echo get_template_directory_uri() . '/src/assets/home/3d-camera-front.png'; ?>"
                        class="absolute lg:-top-40 z-20 w-full h-auto"
                        alt=""
                    >
                </div>
            </div>
            <div class="col-span-1">
                <div class="join join-vertical bg-base-100">
                    <div class="collapse collapse-creatives collapse-arrow join-item border-b border-gray-500 rounded-none">
                        <input type="radio" name="my-accordion-4" checked="checked" />
                        <div class="collapse-title">
                            <h1 class="font-semibold text-3xl">Precision and Clarity</h1>
                        </div>
                        <div class="collapse-content text-sm flex flex-col gap-5">
                            <div class="flex justify-between items-center gap-5">
                                <p class="text-gray-500">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis conse ipsam ipsa deleniti suscipit eos.</p>
                                <div class="bg-primary h-full w-auto">
                                    <img
                                        src="<?php echo get_template_directory_uri() . '/src/assets/home/video-3d-camera.png'; ?>"
                                        class="w-45 h-auto"
                                        alt=""
                                    >
                                </div>
                            </div>
                            <button class="btn-accent w-[200px]">Learn More ></button>
                        </div>
                    </div>
                    <div class="collapse collapse-creatives collapse-arrow join-item border-b border-gray-500 rounded-none">
                        <input type="radio" name="my-accordion-4" />
                        <div class="collapse-title">
                            <h1 class="font-semibold text-3xl">Dynamic Range</h1>
                        </div>
                        <div class="collapse-content text-sm flex flex-col gap-5">
                            <div class="flex justify-between items-center gap-5">
                                <p class="text-gray-500">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis conse ipsam ipsa deleniti suscipit eos.</p>
                                <div class="bg-primary h-full w-auto">
                                    <img
                                        src="<?php echo get_template_directory_uri() . '/src/assets/home/video-3d-camera.png'; ?>"
                                        class="w-45 h-auto"
                                        alt=""
                                    >
                                </div>
                            </div>
                            <button class="btn-accent w-[200px]">Learn More ></button>
                        </div>
                    </div>
                    <div class="collapse collapse-creatives collapse-arrow join-item border-b border-gray-500 rounded-none">
                        <input type="radio" name="my-accordion-4" />
                        <div class="collapse-title">
                            <h1 class="font-semibold text-3xl">Speed <span class="font-anuphan">&</span> Performance</h1>
                        </div>
                        <div class="collapse-content text-sm flex flex-col gap-5">
                            <div class="flex justify-between items-center gap-5">
                                <p class="text-gray-500">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Reiciendis conse ipsam ipsa deleniti suscipit eos.</p>
                                <div class="bg-primary h-full w-auto">
                                    <img
                                        src="<?php echo get_template_directory_uri() . '/src/assets/home/video-3d-camera.png'; ?>"
                                        class="w-45 h-auto"
                                        alt=""
                                    >
                                </div>
                            </div>
                            <button class="btn-accent w-[200px]">Learn More ></button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- NEWSLETTER -->
    <section class="max-w-7xl xl:mx-auto bg-gray-100 mt-10 mb-5">
        <div class="relative p-10 overflow-hidden relative">
                <div class="flex flex-col gap-10 justify-start w-4/7">
                    <h1 class="text-8xl text-primary">Subscribe to our Newsletter</h1>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Error harum nam modi ipsa doloribus deserunt!</p>
                    <div class="flex gap-5">
                        <button class="btn-secondary">Enter your email</button>
                        <button class="btn-accent">Enter your email</button>
                    </div>
                </div>
                <div class="absolute top-0 -right-5">
                    <img
                        src="<?php echo get_template_directory_uri() . '/src/assets/home/video-3d-camera.png'; ?>"
                        class="transform scale-150"
                        alt=""
                    >
                </div>
            </div>
        </div>
    </section>
<!-- </div> -->